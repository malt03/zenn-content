...
もしコメントをされた方が不快に思われるようでしたらご連絡ください。

僕の立場は「SecureEnclaveに閉じたパスキーは便利な普段使い、パスワード+TOTPでバックアップ」というものです。
しかし「同期されたパスキーに一本化し、パスワードは可能な限り削除する」というアプローチがコメントの提案でした。

パスワードもTOTPも、サービス側の漏洩リスクがあります。パスワードに関しては、未だに復号可能な形でDBに保存しているサービスは存在しますし、ハッシュにしても、漏洩時点でブルートフォースによる解析リスクがあります。TOTPも秘密鍵をサービス側が持っている以上、漏洩のリスクがあります。さらにTOTPによる二要素認証はフィッシング耐性があるとは言えません。

一方、パスキーは公開鍵のみがサービス側に保存されるため、サービス側が漏洩しても秘密鍵は安全です。認証をパスキーのみにしておけばサービス側からの漏洩があってもアカウントは乗っ取られません。パスワード+TOTPを「バックアップ」として残すこと自体が、サービス側の漏洩時のリスクになっている、というわけです。
この立場に立つと、パスワードマネージャーが単一障害点になることを許容し、そこを徹底的に守る（YubiKeyで認証、バックアップコードの厳重管理など）という戦略は非常に合理的なトレードオフになります。

正直、この視点は考えたことがありませんでした。
このコメントをいただけただけでも（消えてしまいましたが）この記事を書いた意味がありました。
僕自身、もう一度パスワード管理の方針を考え直してみようと思っています。
